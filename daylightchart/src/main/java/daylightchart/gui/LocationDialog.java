...
/**
 * This class is used for the display of the location editor. The user is allowed to add new
 * locations, and delete or modify existing locations.
 *
 * @author Pradnya Devare, Sualeh Fatehi
 */
public class LocationDialog extends JDialog {

  final class DialogButtonListener implements ActionListener, KeyListener {
    /**
     * {@inheritDoc}
     *
  ...
  @Serial private static final long serialVersionUID = -5161588534167787490L;

  private static final Logger LOGGER = Logger.getLogger(LocationDialog.class.getName());

  /**
   * Show a dialog for location delete, add or edit operation to be performed.
   *
   * @param mainWindow Main window
   * @param operation Operation to perform
   * @return Location, if one was added, edited or deleted, or null if the dialog was canceled
   */
  public static Location showLocationDialog(
      final DaylightChartGui mainWindow, final LocationsListOperation operation) {
    final LocationDialog locationDialog = new LocationDialog(mainWindow, operation);
  ...
  private LocationDialog(
    ...
    timeZone =
        new JComboBox<>(new Vector<TimeZoneDisplay>(DefaultTimezones.getAllTimeZonesForDisplay()));

    ok = new JButton(Messages.getString("DaylightChartGui.LocationEditor.Ok")); // $NON-NLS-1$
    cancel =
        new JButton(Messages.getString("DaylightChartGui.LocationEditor.Cancel")); // $NON-NLS-1$
            ...
            if (!event.isTemporary()) {
              ...
              if (isCurrentLocationValid()) {
                final String timeZoneId =
                    DefaultTimezones.attemptTimeZoneMatch(getCity(), getCountry(), getLongitude());
    ...
    final FormBuilder builder =
        FormBuilder.create()
            .border(new EmptyBorder(15, 15, 15, 15))
            .columns("right:pref, 5dlu, pref")
            .rows("pref, 5dlu, pref, 5dlu, pref, 5dlu, pref, 5dlu, pref, 10dlu, pref");

    builder
        .addLabel(Messages.getString("DaylightChartGui.LocationEditor.City"))
        .labelFor(city)
        .rc(1, 1)
        .add(city)
        .rc(1, 3);
  ...
  private Location getCurrentLocation() {
    return new Location(
        getCity(), getCountry(), getTimeZoneId(), new PointLocation(getLatitude(), getLongitude()));
  }
  ...
  private Longitude getLongitude() {
    ...
    try {
      longitude = new CoordinateParser().parseLongitude(longitudeValue.getText());
    } catch (final us.fatehi.pointlocation6709.parse.ParserException e) {
      LOGGER.log(Level.FINE, e.getMessage(), e);
    }
    return longitude;
