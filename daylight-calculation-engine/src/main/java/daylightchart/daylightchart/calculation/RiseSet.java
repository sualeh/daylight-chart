...
/**
 * Sunrise and sunset at a given location, and a given date. RiseSet is calculated as part of the
 * rise/ set year, and then may be split for creating the daylight bands.
 *
 * @author Sualeh Fatehi
 */
public final class RiseSet implements Serializable, Comparable<RiseSet> {

  static final LocalTime JUST_BEFORE_MIDNIGHT = LocalTime.MIDNIGHT.minusNanos(1);

  static final LocalTime JUST_AFTER_MIDNIGHT = LocalTime.MIDNIGHT.plusNanos(1);

  @Serial private static final long serialVersionUID = 3092668888760029582L;
  ...
  RiseSet(final RawRiseSet rawRiseSet) {
    ...
    if (sunriseRaw == Double.POSITIVE_INFINITY && sunsetRaw == Double.POSITIVE_INFINITY) {
      riseSetType = RiseSetType.all_daylight;
    } else if (sunriseRaw == Double.NEGATIVE_INFINITY && sunsetRaw == Double.NEGATIVE_INFINITY) {
  ...
  private RiseSet(
    ...
    if (sunrise.equals(JUST_AFTER_MIDNIGHT) && !sunset.equals(JUST_BEFORE_MIDNIGHT)
        || !sunrise.equals(JUST_AFTER_MIDNIGHT) && sunset.equals(JUST_BEFORE_MIDNIGHT)) {
      riseSetType = RiseSetType.split;
    } else if (sunrise.equals(JUST_AFTER_MIDNIGHT) && sunset.equals(JUST_BEFORE_MIDNIGHT)) {
  ...
  /**
   * Gets a copy of of this rise/ set, with times adjusted for daylight saving time.
   *
   * @param adjustedForDaylightSavings Whether to adjust the times.
  ...
  /**
   * Gets a copy of this rise/ set, with different sunrise and sunset times.
   *
   * @param sunrise Sunrise
   * @param sunset Sunset
   * @return New rise/ set
